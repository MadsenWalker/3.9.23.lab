...
// const numx1  = numbers.forEach((element) => console.log(element/2))
...
const numx  = numbers.map((element) => (element/2))
...
let citi = cities.filter((city) => city.endsWith("i"))
...
fruits.forEach((fruit, index) => {
  if (fruit.color === "red"){
    return console.log(`The fruit with index ${index} is an apple.`)
  } else if (fruit.color === "orange"){
    return console.log(`The fruit with index ${index} is an orange.`)
} else {
...
const totalMeat = foods.filter((meat) => {
  return meat.tags.includes("meat")}).reduce((total, meat) => 
 total + meat.price, 0)
